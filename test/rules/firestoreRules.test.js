...
describe('viewerGrants — 설문별 결과 열람 grant', () => {
  const OWNER = 'grant-owner'; const VIEWER = 'grant-viewer'; const INACTIVE = 'grant-inactive'; const OTHER = 'grant-other'; const surveyId = 'grant-survey';
  const fs = (uid) => testEnv.authenticatedContext(uid, { email: `${uid}@yeongjung.or.kr` }).firestore();
  const grant = (uid) => ({ uid, grantedByUid: OWNER, createdAt: serverTimestamp() });
  async function seed(status = 'active') { await Promise.all([seedUserDoc(OWNER,{uid:OWNER,email:`${OWNER}@yeongjung.or.kr`,role:'creator',status:'active'}),seedUserDoc(VIEWER,{uid:VIEWER,email:`${VIEWER}@yeongjung.or.kr`,role:'viewer',status}),seedUserDoc(INACTIVE,{uid:INACTIVE,email:`${INACTIVE}@yeongjung.or.kr`,role:'viewer',status:'inactive'}),seedUserDoc(OTHER,{uid:OTHER,email:`${OTHER}@yeongjung.or.kr`,role:'creator',status:'active'})]); await testEnv.withSecurityRulesDisabled(async (ctx)=>{await setDoc(doc(ctx.firestore(),'surveys',surveyId),{title:'공유 설문',status:'published',ownerUid:OWNER,responseCount:1});await setDoc(doc(ctx.firestore(),'responses','grant-response'),minimalResponsePayload(surveyId,{surveyOwnerUid:OWNER}));await setDoc(doc(ctx.firestore(),'survey_reports','grant-report'),{surveyId,title:'보고서'});}); }
  it('owner는 active 대상에게만 grant를 만들고, 대상은 설문·응답·보고서를 읽는다', async()=>{await seed(); await assertSucceeds(setDoc(doc(fs(OWNER),'surveys',surveyId,'viewerGrants',VIEWER),grant(VIEWER))); await assertSucceeds(getDoc(doc(fs(VIEWER),'surveys',surveyId))); await assertSucceeds(getDoc(doc(fs(VIEWER),'responses','grant-response'))); await assertSucceeds(getDoc(doc(fs(VIEWER),'survey_reports','grant-report')));});
  it('inactive/nonexistent 대상 grant와 viewer write는 차단되고 grant 제거 후 즉시 읽기가 막힌다', async()=>{await seed(); await assertFails(setDoc(doc(fs(OWNER),'surveys',surveyId,'viewerGrants',INACTIVE),grant(INACTIVE))); await assertFails(setDoc(doc(fs(OWNER),'surveys',surveyId,'viewerGrants','missing'),grant('missing'))); await testEnv.withSecurityRulesDisabled(async(ctx)=>setDoc(doc(ctx.firestore(),'surveys',surveyId,'viewerGrants',VIEWER),{uid:VIEWER,grantedByUid:OWNER,createdAt:new Date()})); await assertFails(updateDoc(doc(fs(VIEWER),'surveys',surveyId),{title:'변조'})); await assertFails(updateDoc(doc(fs(VIEWER),'responses','grant-response'),{adminNote:'변조'})); await assertFails(setDoc(doc(fs(VIEWER),'surveys',surveyId,'viewerGrants',OTHER),{uid:OTHER,grantedByUid:VIEWER,createdAt:serverTimestamp()})); await testEnv.withSecurityRulesDisabled(async(ctx)=>deleteDoc(doc(ctx.firestore(),'surveys',surveyId,'viewerGrants',VIEWER))); await assertFails(getDoc(doc(fs(VIEWER),'responses','grant-response')));});
  it('users status가 inactive이면 남아 있는 grant도 즉시 무효다', async()=>{await seed('inactive'); await testEnv.withSecurityRulesDisabled(async(ctx)=>setDoc(doc(ctx.firestore(),'surveys',surveyId,'viewerGrants',VIEWER),{uid:VIEWER,grantedByUid:OWNER,createdAt:new Date()})); await assertFails(getDoc(doc(fs(VIEWER),'responses','grant-response')));});
  it('viewer는 grant 설문의 허용된 다운로드 audit만 남길 수 있다', async()=>{await seed();await testEnv.withSecurityRulesDisabled(async(ctx)=>setDoc(doc(ctx.firestore(),'surveys',surveyId,'viewerGrants',VIEWER),{uid:VIEWER,grantedByUid:OWNER,createdAt:new Date()}));const base={surveyId,responseId:null,actor:{uid:VIEWER,email:`${VIEWER}@yeongjung.or.kr`,displayName:'조회자'},metadata:{downloadType:'raw',totalCount:1},createdAt:serverTimestamp()};for(const action of ['responses_csv_downloaded','statistics_excel_downloaded','report_docx_downloaded','report_print_clicked'])await assertSucceeds(addDoc(collection(fs(VIEWER),'audit_logs'),{...base,action}));});
