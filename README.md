...
신청형 설문은 정원과 함께 **현재 신청 / 정원**을 우선 표시합니다. 응답 관리 화면에서는
현재 신청, 취소, 전체 접수를 서로 다른 응답 이력 기준으로 구분합니다.
...
응답 삭제는 보관형 처리이며 admin/super_admin은 전체 설문, 제작자와 legacy owner는 자기
설문의 응답만 삭제할 수 있습니다. 취소된 신청을 삭제해도 정원·quota·lock은 다시 반환하지 않습니다.

### 응답 원본 다운로드와 설문 수정

Raw Excel/CSV 다운로드는 현재 문항뿐 아니라 기존 응답에만 남아 있는 이전 문항 답변도 별도
`[이전 문항]` 열로 보존합니다. 새 문항을 추가한 경우 이전 응답의 해당 셀은 빈칸인 것이 정상입니다.
반대로 문항을 삭제 후 다시 만들면 제목이 같아도 새 문항으로 취급되므로, 과거 답변은 현재 문항과
합치지 않고 별도 열로 표시됩니다. 통계·차트·결과보고서는 비교 가능성을 위해 계속 현재 설문 문항
구조만 기준으로 집계합니다.

활성 내부 직원은 화면 우측의 **의견 보내기**에서 오류·개선 제안·문의·기타 의견을 남기고 `/feedback`에서 처리 상태와 관리자 답변을 확인할 수 있습니다. 관리자는 `/admin/feedback`에서 접수 → 확인 시작 → 처리 내용 작성 → 완료 순서로 처리합니다. 의견과 답변에는 주민 개인정보나 설문 응답 원문을 입력하거나 자동 첨부하지 않습니다.

`staff_directory`는 결과 공유 설정에서 직원 선택을 돕는 최소 projection(uid, 이름, 활성 여부)입니다. 기존 active 직원의 initial backfill은 완료됐으며, 이후 로그인과 관리자 사용자 변경에서 projection을 동기화합니다. users 원본 read 권한을 확대하거나 Viewer 권한의 정본으로 사용하지 않습니다.
