...
향후 설문별 결과 공유를 위한 `staff_directory`는 직원 선택용 최소 projection(uid, 이름, 활성 여부)입니다. 기존 active 직원의 initial backfill은 완료됐으며, 이후 로그인과 관리자 사용자 변경에서 projection을 동기화합니다. users 원본 read 권한이나 Viewer 결과 접근은 아직 확대하지 않습니다.
