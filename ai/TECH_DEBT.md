...
- 템플릿 생성 후 질문/섹션 매칭 결과를 확인하는 개발용 체크 함수 추가(2026-07-10:
  `test/applyTemplateIdRemap.test.js`로 템플릿 적용/섹션 복제 시 문항 ID remap
  검증은 테스트로 커버했다 — KI-016. 빌더 UI 안에서 저장 전 실시간으로 매칭
  결과를 보여주는 개발용 체크 함수 자체는 아직 없다)
