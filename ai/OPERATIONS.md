...
- 응답 관리 화면의 `담당자 인계`에서 다른 활성 직원을 선택합니다. 목록은 `staff_directory`의 이름만
  사용하므로 이메일을 확인하거나 입력할 필요가 없습니다.
- 인계 후 새 담당자가 설문·응답·결과보고서·결과 같이 보기를 관리합니다. 기존 담당자는 기본적으로
  권한을 잃으며, 필요한 경우 체크박스로 Viewer 결과 조회만 남길 수 있습니다.
- 실제 주민 응답·정원·quota·lock·공개 URL은 인계 대상이 아닙니다. 인계 후 새 담당자와 기존 담당자로
  각각 수정 가능/불가 여부를 테스트 설문에서 확인합니다.
...
- admin/super_admin은 전체 설문, 제작자 및 legacy owner는 자기 설문 응답만 삭제할 수 있습니다.
- submitted 응답 삭제는 설문 응답 수와 선택지·연령 정원 집계를 함께 조정합니다.
- cancelled 응답 삭제는 이력만 제외하며, 취소 때 반환된 정원·quota·lock을 다시 변경하지 않습니다.
