...
기능·권한·데이터 계약·Firestore Rules·배포 절차가 변경되는 작업은 완료 전에 README와 관련
운영문서의 정합성을 확인한다. 설명이 실제 동작과 달라졌다면 같은 release 범위에서 문서를
갱신하며, 테스트 추가나 내부 refactor처럼 운영 설명이 변하지 않은 경우에는 기계적으로
문서를 변경하지 않는다.
