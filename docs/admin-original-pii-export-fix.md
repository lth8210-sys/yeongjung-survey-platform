...
- ~~`functions/src/submitResponse.js`(Structure A 서버 경로)에 동일한
  버그 패턴이 그대로 남아 있다.~~ **정정(2026-07-14 후속 세션)**: 이 판단은
  틀렸다 — 168~199번째 줄을 재검토한 결과, `protectedAnswers`/
  `respondentName` 계산은 이미 `encryptIdentityAndAnswerPii()`가 KMS
  암호화에 **전부 성공했을 때만** 도달하는 코드였다(실패 시
  `Promise.all`이 reject해 함수 자체가 throw하고, 호출부가 그대로
  `HttpsError`로 전파해 트랜잭션을 시작하지 않는다 — "평문 fallback
  금지"가 처음부터 올바르게 구현돼 있었다). 즉 이 파일에는 애초에
  Hosting 경로와 같은 "무조건 마스킹 저장" 버그가 없었다. 다만 사용자가
  명시한 "즉시 실패가 아니라 자동 재시도(1~2회) 후 실패"라는 UX
  정책은 없었다(즉시 1회 실패로 포기) — 이 부분은 후속 세션에서
  `encryptFieldWithRetry()`(재시도)와 `verifyPiiPreservedBeforeSave()`
  (별도 안전망)를 추가해 보완했다. 자세한 내용은
  `docs/pii-encryption-architecture.md` §0.2, §11.12 참고. 코드·테스트만
  변경했고 배포하지 않았다(여전히 `VITE_USE_SERVER_RESPONSE_SUBMISSION=false`).
